...
**PR 本文と同じ基準。コード内に「なぜこう設計したか」を書かない。** 設計の理由は決定記録にあり、
...
- **doc へのポインタも書かない**（`決定 0004 参照` など）。理由を書かないのだから参照も要らない。
...
- ❌ `# key は games/ を含める（決定 0001 参照）`
- ❌ `詳細は docs/future/ingest.md を見る`

例外は 2 つ。**決定記録どうしの相互リンク**（supersede 関係や前提の連鎖を辿る必要がある）と、
**`docs/specs/` の末尾に置く決定記録への導線**（spec は「何を保証するか」だけを書き、
理由は決定記録にあるため）。どちらも番号ではなく内容で指す。

#### 会話でも同じ

ユーザーへの説明で `決定 0004 にある通り` と言わない。**内容を直接言う**
...
| 決定記録 | **書き換えない** | 新しい番号で追加し、古い方を `Superseded` にする |
| `docs/future/` | 気にしない（構想なので） | 実装したら消して spec / 決定記録へ移す |
...
決定記録は「一度潰れた案」の記録でもある。**再提案する前に該当の 1 本を読み、
