...
**同時に走らせるのは 1 本だけ**（CLAUDE.md「小さく作る・並列にしない」）。依存が無くても並行しない。
1 セッションで複数タスクを抱えるのも同じ理由で避ける。

小見出しが大きすぎるときはさらに割る。1 ブランチ = 1 PR = 「1 つのことができるようになる」まで。
...
**タスク識別子（`A1` / `B1` …）も本文に書かない**（branch 名と title にある）。
**diff を読めば分かることは書かない。**

## 着手順

`docs/tasks.md` の Phase が着手順。**1 本ずつ、マージしてから次を切る。**
